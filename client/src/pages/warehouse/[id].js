...
export default function Page({ warehouseId }) {
  ...
  const { detailWarehouse, setDetailWarehouse, categories } = useContext(DataContext);
  ...
  useEffect(() => {
    if (detailWarehouse?.stockItems) {
      const sortedItems = [...detailWarehouse.stockItems].sort((a, b) => a.stock - b.stock);
              ...
              {sortedStockItems?.map((item) => (
                <HStack display={'flex'} justifyContent={'space-between'} w={'100%'} key={item.id} py={{ base: 2, md: 3 }} px={{ base: 4, md: 6 }} border={'1px'} borderColor={'gray.300'} borderRadius={'lg'}>
                  <Flex flexDirection={'row'} gap={4}>
                    <Image src={item.items.imageUrl} aspectRatio={'1/1'} w={12} h={12} mr={7} objectFit={'contain'} borderRadius={'xl'} />
                    <Box>
                      <Text fontSize={'lg'} fontWeight={'semibold'}>
                        {item.items.name} - {categoriesName(item.items.categoriesId)}
                      </Text>
                  ...
                  <Flex flexDirection={'column'} w={'13%'} alignItems={'flex-start'} pl={3}>
                    ...
                    <Text fontSize={'lg'} fontWeight={'bold'} style={{ color: item.stock === 0 ? 'red' : item.stock > 0 && item.stock < 10 ? 'orange' : 'green' }}>
                      {item.stock}
                    </Text>
                  </Flex>
                </HStack>
